    ...
    public class LevelSpawnedFogBeaconManager : GenericExpeditionDefinitionManager<LevelSpawnedFogBeaconDefinition>
    {
        ...
        private void Build(LevelSpawnedFogBeaconDefinition def)
        {
            foreach (var d in def.SpawnedBeaconsInZone)
            {
                ...
                {
                    EOSLogger.Error($"LevelSpawnedFogBeaconManager: WorldEventObjectFilter '{d.WorldEventObjectFilter}' is either unassigned or has already been assigned.");
                    continue;
                }
                ...
                EOSLogger.Debug($"LevelSpawnedFogBeaconManager: spawned '{d.WorldEventObjectFilter}' in {def.GlobalZoneIndexTuple()}, Area_{(char)('A' + d.AreaIndex)}");
            }
        }

        public LevelSpawnedFogBeaconSettings GetLSFBDef(HeavyFogRepellerGlobalState h) => LSFBGlobalStatesSet.TryGetValue(h.Pointer, out var def) ? def : null;
        ...
        public void ToggleLSFBState(string worldEventgObjectFilter, bool enable)
        {
            ...
            {
                EOSLogger.Error($"ToggleLSFBState: '{worldEventgObjectFilter}' is not defined");
                return;
            }
        ...
        {
            ToggleLevelSpawnedFogBeaconState = 922 // TODO: confirm
        }

        private static void ToggleLevelSpawnedFogBeaconState(WardenObjectiveEventData e)
        {
            Current.ToggleLSFBState(e.WorldEventObjectFilter, e.Enabled);
        }
    }
}
